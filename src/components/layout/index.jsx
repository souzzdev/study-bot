export function Header({ onMenuClick, savedToast }) {
  return (
    <>
      {savedToast && (
        <div className="toast">✓ Progresso salvo</div>
      )}
    ...
    <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#111318", borderTop: "1px solid #1e2030", display: "flex", zIndex: 10 }}>
      {NAV_TABS.map((tab) => (
        <button key={tab.id} onClick={() => onNavigate(tab.id)}
          style={{ flex: 1, background: "none", border: "none", padding: "12px 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", transition: "color 0.15s", color: page === tab.id ? "#38bdf8" : "#555", fontFamily: "inherit" }}>
          <span style={{ fontSize: 18 }}>{tab.icon}</span>
          <span style={{ fontSize: "0.68rem", fontWeight: 600, letterSpacing: "0.04em" }}>{tab.label}</span>
        </button>
      ))}
    </div>
  );
